...
- **Crypto/encoding deps — no `rsa`; `rand`/`sha2`/`base64` majors and `p12-keystore` 0.2.x pinned on purpose.** `cert.rs` uses `rcgen` on the `aws_lc_rs` backend specifically to keep `rsa` (RUSTSEC-2023-0071) out of the graph. The `rand`/`sha2`/`p12-keystore` pins match what `oauth2` 5 + Tauri 2 resolve; bumping one nothing else follows only *adds* a duplicate major. `base64` is held for a DIFFERENT reason — 0.23 defaults `simd-unsafe` on, and the graph already carries 0.23 via `rcgen`→`pem`, so the duplicate-major argument does not apply there. Rationale + drop conditions live in `dependabot.yml`'s `ignore` blocks.

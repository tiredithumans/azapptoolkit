...
- **Tenant-scoped caches — cross-tenant leakage is the #1 footgun.** Cache keys are `{tenant_id}|{kind}`; never unscoped. On sign-out, `invalidate_prefix(kind, "{tenant_id}|")` for **every** kind. Index entries are pinned (`put_index`/`put_typed_index`); the SP index is *typed* — read only via `sp_index_hit`/`sp_index_store` (plain `get` = silent miss + rescan). Never pin a per-object key; bound bulk seeding by `Cache::capacity_for`. Details: [caching-and-search.md](docs/architecture/caching-and-search.md).
...
- **Batched Graph fan-out + adaptive throttle.** Heavy per-object fan-outs (security audit, DR backup) use Graph JSON batching (`client.batch_get_json[_with_headers]`, 20 GETs/POST) + the shared `ConcurrencyThrottle` attached via `ThrottleGuard::attach`. Reuse both for any new heavy fan-out; never hand-roll a tracker or a raw per-item loop. Whole-batch failures degrade to per-object reads. `/applications` scans page at `DEFAULT_APP_PAGE_SIZE` (999); `$count`/`$orderby` belong to `$search` alone, as `$expand` + advanced query is unsupported and fails *silently* (`$expand` caps at 20 items, no `nextLink`). Details: [caching-and-search.md](docs/architecture/caching-and-search.md).
...
5. **Frontend GUI tests** *(browser-gated; in `verify-full`/CI, not `verify`)* — `just web-itest`: real Leptos views in a headless browser, Tauri IPC mocked. Tests are `tests/gui/<view>.rs` **modules**, grouped into shard binaries (`tests/gui_N.rs`) via `#[path] mod`; harness in `web-rs/src/test_support/`. **Two coupled perf levers:** (a) `[profile.test] strip = "debuginfo"` — each debug test wasm is ~96% DWARF the runner would otherwise decode (~24s/binary); keep `"debuginfo"`, never `strip = true` (kills the `name` section → anonymous panic traces). (b) **sharding** — a single merged binary (~78 MB) exceeds what headless Chrome will instantiate, so tests split into shards kept **under ~52 MB each** (measure: `cargo test --no-run … && ls -la target/…/deps/gui_*.wasm`). Adding a view test: drop in `tests/gui/<view>.rs`, add a `#[path] mod` to the **smallest** shard, re-measure; if all shards are at the ceiling add a new `gui_N.rs` rather than push one over. **Drift: shards 1-3 are ~59-64 MB and can SIGKILL chromedriver locally.** Tests in a shard share one page, relying on Leptos disposing each `mount_view` (plus teleported Thaw overlays) on unmount. **Do NOT make `test_support::reset()` clear `document.body`** — the runner scrapes results from the page DOM, so wiping it makes the shard report nothing (`Failed to detect test as having been run`, 60s timeout). Renaming a CSS class / aria-label / on-screen text a test references passes local `verify` but fails CI — the `web-test-strings-check.sh` hook warns at edit time.

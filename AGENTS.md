...
- **Scoped grants reuse shared cores.** Exchange + SharePoint both grant scoped access *before* stripping org-wide, so a failure never strands the principal. Exchange scope source: `azapptoolkit_<app_id>` mail-enabled security group (`group_name_for`); membership changes **don't** invalidate caches. The management **scope** is named separately — `scope_name_for` → `app_scope_<app_id>` (distinct from the group so they never collide; AAP migration takes an optional per-app override). Details: [scoping-and-audit.md](docs/architecture/scoping-and-audit.md).
...
- **Per-tenant operator defaults live in `settings.json` too.** `UserSettings.tenant_defaults` (keyed by tenant id) holds default owners, SSO notification emails, the AAP scope-name pattern, and vault bindings; types are in `azapptoolkit-core::defaults` (ungated — the wasm frontend uses them as the `get_tenant_defaults`/`set_tenant_defaults` IPC payload, while `settings` persistence stays native-only). `settings.json` now has **two writers** (`commands::config` + `commands::defaults`); both read-modify-write via `UserSettings::stored`. `set_tenant_defaults` only writes the operator-editable fields — it **preserves `default_vault`/`app_vaults`** (owned by the rotation flow, which persists a per-app binding via `UserSettings::set_app_vault_binding`; vaults discovered by `list_available_key_vaults`) via `apply_tenant_defaults`. Owner-search reuse: `components::owner_picker::OwnerPicker` (Settings page); the existing Owners tabs keep their own flows. `get_tenant_defaults` is an infallible `invoke` → registered in `demo::register_fixtures` (see the demo footgun below).

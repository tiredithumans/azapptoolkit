...
- **Repointing a management scope is an explicit action, and fail-closed.** `ensure_management_scope` is create-only; `set_management_scope_filter` is the sole filter mutator and Exchange applies it to **every** role assignment on that scope, so a grant only *warns* on a group-set mismatch. Consolidating a legacy group onto the managed one (`consolidate_scope_group` — AAP migration + `move_exchange_scope_to_managed_group`) repoints only when every source mailbox is **verified present** in the managed group; an *empty* source group counts as unreadable (`Get-DistributionGroupMember` is silent for M365 groups). Details: [scoping-and-audit.md](docs/architecture/scoping-and-audit.md).

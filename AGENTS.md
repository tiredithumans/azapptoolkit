...
- **Scope-aware audit risk.** `score_application` reads `AppPermissions.mail_scopes` (empty map = org-wide, so an unresolved probe never under-reports). A **legacy AAP** verdict comes from ONE tenant-wide policy read per run and is its OWN finding + migrate Fix — same reduced weight, but never the `SCOPED_VIA_RBAC` healthy one. SharePoint scoping is name-based, no live call. Badges: `web-rs/components/scope_badge.rs`.
...
- **Repointing a management scope is an explicit action, and fail-closed.** `ensure_management_scope` is create-only; `set_management_scope_filter` is the sole filter mutator and Exchange applies it to **every** role assignment on that scope, so a grant only *warns* on a group-set mismatch. Consolidating a legacy group onto the managed one (`consolidate_scope_group` — AAP migration + `move_exchange_scope_to_managed_group`) repoints only when every source mailbox is **verified present** in the managed group; an *empty* source group counts as unreadable (`Get-DistributionGroupMember` is silent for M365 groups). The retired group is then **named** (`retired_groups`), and `delete_exchange_scope_group` is irreversible — it re-checks live and refuses the managed group, any live reference, or an **incomplete** check. Details: [scoping-and-audit.md](docs/architecture/scoping-and-audit.md).

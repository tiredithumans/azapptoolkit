...
- **Tenant-scoped caches — cross-tenant leakage is the #1 footgun.** Keys are `{tenant_id}|{kind}`, never unscoped; sign-out calls `invalidate_prefix` for **every** kind. The two tenant-wide indexes are *typed* and pinned — read via `sp_index_*`/`app_name_index_*`/`indexes_cached` (plain `get` = silent miss + rescan). Never pin a per-object key (pinned by `repo_invariants.rs`); bound bulk seeding by `capacity_for`. Details: [caching-and-search.md](docs/architecture/caching-and-search.md).

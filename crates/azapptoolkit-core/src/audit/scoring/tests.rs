//! Table-driven scoring tests (`super`), kept out of `scoring.rs` so the
//! production code reads in one pass. Each rule cites the legacy PowerShell
//! `file:line` it was ported from.
...
fn sp_scoring_never_emits_credential_or_manifest_findings() {
    ...
    let item = score_service_principal(&old, &sp_perms(&["Mail.Read", "Mail.ReadWrite"]), now());
...
fn rbac_and_legacy_scopes_on_one_app_split_into_both_findings() {
    ...
    assert!(issue_with("Mailbox access scoped via RBAC for Applications:").contains("Mail.Send"));
